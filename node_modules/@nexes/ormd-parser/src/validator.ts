...
import { ContextBundle, ValidationResult } from './types';

/**
 * JSON Schema validator for ContextBundle format
 */
export class ContextBundleValidator {
  private static ajv = new Ajv({ allErrors: true });

  private static contextBundleSchema: JSONSchemaType<ContextBundle> = {
    type: 'object',
    properties: {
  ...
  private static validate = this.ajv.compile(this.contextBundleSchema);
